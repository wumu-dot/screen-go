...
onMounted(() => {
  mainWindowListener()
  // 定时检查 localStorage 中的图片数据（绕过 IPC 问题）
  pollTimer = setInterval(() => {
    const stored = getItem('screenDataBase64')
    if (stored && configStore.screenData.baseData !== stored) {
      configStore.screenData.baseData = stored
    }
  }, 500)
})
